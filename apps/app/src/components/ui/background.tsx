...
function mixColors(first: RgbaColor, second: RgbaColor, amount: number): RgbaColor {
...
function samplePointerTrailInfluence(blobs: ActivePointerTrailBlob[], u: number, v: number) {
  if (blobs.length === 0) {
    return 0;
  }
  ...
  for (const blob of blobs) {
    ...
    const gaussian = Math.exp(-((deltaX * deltaX + deltaY * deltaY) / (2 * radius * radius)));
...
export function Background({
  ...
  useEffect(() => {
    ...
    if (!(container && fieldCanvas && asciiCanvas) || typeof window === "undefined") {
      return;
    }
    ...
    const allocateBuffers = () => {
      ...
      if (offscreenCanvas.width !== grid.fieldCols || offscreenCanvas.height !== grid.fieldRows) {
        offscreenCanvas.width = grid.fieldCols;
        offscreenCanvas.height = grid.fieldRows;
      }

      fieldImageData = offscreenContext.createImageData(grid.fieldCols, grid.fieldRows);
    };
    ...
    const renderFrame = (elapsedMs: number) => {
      if (
        !(grid && offscreenContext && fieldContext && asciiContext && fieldImageData) ||
        width === 0 ||
        height === 0
      ) {
        return;
      ...
      const safeTime = elapsedMs * 0.000_085 * Math.max(0.25, resolvedConfig.speed);
      ...
      const pointerTargetCenterX = pointerActive && pointerEnabled ? pointerTargetX : 0.5;
      const pointerTargetCenterY = pointerActive && pointerEnabled ? pointerTargetY : 0.5;
      const pointerFollow = pointerActive && pointerEnabled ? 0.085 : 0.028;
      pointerCurrentX += (pointerTargetCenterX - pointerCurrentX) * pointerFollow;
      pointerCurrentY += (pointerTargetCenterY - pointerCurrentY) * pointerFollow;
      ...
      if (pointerTrailEnabled) {
        for (let index = 0; index < pointerTrailPool.maxBlobs; index += 1) {
          ...
          const blobState = getPointerTrailBlobState({
            ...
            radius: pointerTrailPool.radius[index] ?? resolvedConfig.pointerTrailRadius,
      ...
      for (let y = 0; y < grid.fieldRows; y += 1) {
        ...
        for (let x = 0; x < grid.fieldCols; x += 1) {
          ...
          const firstWarp =
            Math.sin((u * 2.6 + v * 0.8 + safeTime * 0.95 + seedOffsets.waveA) * TAU) * 0.045 +
            Math.cos((v * 3.1 - safeTime * 0.62 + seedOffsets.waveB) * TAU) * 0.018;
          const secondWarp =
            Math.cos((v * 2.3 - u * 0.65 + safeTime * 0.82 + seedOffsets.waveC) * TAU) * 0.04 +
            Math.sin((u * 3.4 - safeTime * 0.44 + seedOffsets.waveD) * TAU) * 0.024;
          ...
          for (const emitter of emitters) {
            const centerX =
              emitter.centerX +
              Math.sin(safeTime * emitter.speed + emitter.phase) * emitter.orbitRadiusX;
            const centerY =
              emitter.centerY +
            ...
            combinedField +=
              emitter.weight * Math.exp(-(deltaX * deltaX + deltaY * deltaY) / emitter.spread);
          }
          ...
          const ribbon =
            ...
            0.5 *
              Math.sin(
                (sampleU * 4.8 + sampleV * 1.35 + safeTime * 1.08 + seedOffsets.ribbonA) * TAU,
              );
          const veil =
            0.5 +
            0.5 *
              Math.cos(
                (sampleV * 3.7 - sampleU * 1.72 - safeTime * 0.9 + seedOffsets.ribbonB) * TAU,
              );
          const turbulence =
            0.5 + 0.5 * Math.sin((sampleU + sampleV + safeTime * 0.42 + seedOffsets.ribbonC) * TAU);
          const finalValue =
            combinedField * (0.72 + ribbon * 0.22) + veil * 0.16 + turbulence * 0.08;
          ...
          const trailInfluence =
            activePointerTrailBlobs.length > 0
              ...
              : 0;
          const shapedValue =
            finalValue + trailInfluence * resolvedConfig.pointerTrailIntensity * 0.12;
      ...
      for (let index = 0; index < fieldValues.length; index += 1) {
        const density = normalizeFieldValue(fieldValues[index] ?? 0, minimumValue, maximumValue);
        ...
        const accentMix =
          density ** 1.8 * 0.42 + trailInfluence * resolvedConfig.pointerTrailIntensity * 0.14;
        const baseColor = mixColors(palette.background, palette.neutral, neutralMix);
        const finalColor = mixColors(baseColor, palette.accent, accentMix);
        const alpha = clampUnit(
          resolvedConfig.fieldOpacity * (0.12 + density * 0.88 + trailInfluence * 0.18),
        );
      ...
      for (let y = 0; y < grid.asciiRows; y += 1) {
        ...
        for (let x = 0; x < grid.asciiCols; x += 1) {
          ...
          const trailInfluence =
            activePointerTrailBlobs.length > 0
              ? samplePointerTrailInfluence(activePointerTrailBlobs, sampleX, sampleY)
              : 0;
          const density = clampUnit(
            ambientDensity + trailInfluence * (0.62 + resolvedConfig.pointerTrailIntensity * 0.28),
          );
          const contrastedDensity = clampUnit(
            (density - 0.5) * (resolvedConfig.strength * (1 + trailInfluence * 1.25)) + 0.5,
          );
          if (contrastedDensity < Math.max(0.02, 0.06 - trailInfluence * 0.035)) {
            continue;
          }

          const mappedDensity = resolvedConfig.reverse ? contrastedDensity : 1 - contrastedDensity;
          ...
          const glyphAlpha = clampUnit(
            0.34 + density * 0.58 + trailInfluence * resolvedConfig.pointerTrailIntensity * 0.55,
          );
          const glyphAccentMix = clampUnit(
            density * 0.12 + trailInfluence * (0.92 + resolvedConfig.pointerTrailIntensity * 0.18),
          );
          const glyphColor = mixColors(palette.asciiBase, palette.accent, glyphAccentMix);
    ...
    const updateSize = () => {
      ...
      width = nextWidth;
      height = nextHeight;
      devicePixelRatio = Math.min(2, Math.max(1, Math.round(window.devicePixelRatio || 1)));
      ...
      updateCanvasElementSize(fieldCanvas, nextWidth, nextHeight, devicePixelRatio);
      updateCanvasElementSize(asciiCanvas, nextWidth, nextHeight, devicePixelRatio);
      allocateBuffers();
      if (isVisible) {
        renderFrame(lastElapsedMs);
      }
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const deltaX = hasLastPointerSample ? pointerLocalX - lastPointerSampleX : 0;
      const deltaY = hasLastPointerSample ? pointerLocalY - lastPointerSampleY : 0;
      const deltaTime = hasLastPointerSample ? Math.max(16, now - lastPointerSampleMs) : 16;
      const velocityMagnitude = Math.hypot(deltaX, deltaY);
      const distanceSinceSpawn = hasLastTrailSpawn
        ? Math.hypot(pointerLocalX - lastTrailSpawnX, pointerLocalY - lastTrailSpawnY)
        : 0;
      ...
      if (
        ...
        const normalizedVelocityX =
          rect.width > 0
            ? (deltaX / rect.width) * (16 / deltaTime) * (0.42 + speedFactor * 0.64)
            : 0;
        const normalizedVelocityY =
          rect.height > 0
            ? (deltaY / rect.height) * (16 / deltaTime) * (0.42 + speedFactor * 0.64)
            : 0;

        writePointerTrailBlob(pointerTrailPool, {
          intensity: resolvedConfig.pointerTrailIntensity * (0.78 + speedFactor * 0.22),
          radius: resolvedConfig.pointerTrailRadius * (0.92 + speedFactor * 0.16),
          spawnedAtMs: now,
