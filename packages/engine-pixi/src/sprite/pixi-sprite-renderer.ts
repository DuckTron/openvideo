 ...
 * Creates textures directly from VideoFrame/ImageBitmap sources for zero-copy GPU upload
 */
export class PixiSpriteRenderer {
   ...
   * @param frame VideoFrame, ImageBitmap, Texture, or null to render
   *              VideoFrames are consumed (closed) after texture upload.
   */
  async updateFrame(frame: VideoFrame | ImageBitmap | Texture | null): Promise<void> {
    ...
    const isTexture =
      ...
      (frame && typeof (frame as any).source !== "undefined" && !(frame instanceof VideoFrame));

    if (isTexture) {
      // Validate texture dimensions
      if (frame.width === 0 || frame.height === 0) {
        console.warn("PixiSpriteRenderer: Texture has zero dimensions", frame.width, frame.height);
        return;
      }
    ...
    // Fast path: VideoFrame or ImageBitmap → Texture.from() directly
    // Avoids the old Canvas 2D drawImage intermediate copy
    const width = (frame as any).width;
    const height = (frame as any).height;

    if (typeof width !== "number" || typeof height !== "number" || width <= 0 || height <= 0) {
      console.warn("PixiSpriteRenderer: Invalid frame dimensions", width, height);
      if (frame instanceof VideoFrame) frame.close();
      return;
    }

    // Destroy previous texture to avoid GPU memory leak
    if (this.texture != null) {
      this.texture.destroy(true);
      this.texture = null;
    }

    // Create texture directly from the frame source (VideoFrame or ImageBitmap)
    // This uploads to GPU in a single copy instead of frame→canvas→GPU
    this.texture = Texture.from(frame as any);
    ...
    // Close VideoFrame after GPU upload — it is single-use
  ...
  private applySpriteTransforms(): void {
    ...
    const { flip, center, width, height, angle, opacity, zIndex, renderTransform } = this.sprite;
    ...
    if (this.animationContainer) {
      ...
      this.animationContainer.angle = (flip?.x || flip?.y ? -1 : 1) * angleOffset;
    ...
    if (isMirrored) {
      ...
      if (this.isMirrorActive) {
        ...
        if (this.mirrorContainer && this.animationContainer) {
          const idx = this.animationContainer.getChildIndex(this.mirrorContainer);
  ...
  private applyStroke(style: any): void {
    const stroke = style.stroke;
    if (stroke && stroke.width > 0) {
      if (this.strokeGraphics == null) {
        this.strokeGraphics = new Graphics();
        this.strokeGraphics.label = "StrokeGraphics";
        (this.strokeGraphics as any)._calculateBounds = () => {}; // Do not affect bounding box
      ...
      if (borderRadius > 0) {
        ...
        this.strokeGraphics.roundRect(-width / 2, -height / 2, width, height, r);
      } else {
        this.strokeGraphics.rect(-width / 2, -height / 2, width, height);
      }
  ...
  private applyShadow(style: any): void {
    ...
    if (
      ...
      width > 0 &&
      height > 0 &&
      (shadow.blur > 0 || (shadow.offsetX ?? 0) !== 0 || (shadow.offsetY ?? 0) !== 0)
    ) {
      if (this.shadowContainer == null) {
        this.shadowContainer = new Container();
        this.shadowContainer.label = "ShadowContainer";
        (this.shadowContainer as any)._calculateBounds = () => {}; // Do not affect bounding box
        this.shadowGraphics = new Graphics();
        this.shadowGraphics.label = "ShadowGraphics";
        (this.shadowGraphics as any)._calculateBounds = () => {}; // Do not affect bounding box
      ...
      if (borderRadius > 0) {
        const r = Math.min(borderRadius, width / 2, height / 2);
        this.shadowGraphics!.roundRect(-width / 2, -height / 2, width, height, r);
      } else {
      ...
      if (blur > 0) {
        if (!this.shadowContainer.filters || this.shadowContainer.filters.length === 0) {
        ...
        const worldScale = this.root
          ? Math.sqrt(this.root.worldTransform.a ** 2 + this.root.worldTransform.b ** 2)
          : 1;
  ...
  private applyBlur(blur: number): void {
    ...
    if (typeof blur !== "number" || !isFinite(blur) || blur <= 0) {
      if (this.animationContainer.filters && this.animationContainer.filters.length > 0) {
  ...
  private applyMotionBlur(motionBlur: number): void {
    ...
    // Safety check for valid number
    if (typeof motionBlur !== "number" || !isFinite(motionBlur) || motionBlur <= 0) {
      if (this.animationContainer.filters) {
        this.animationContainer.filters = this.animationContainer.filters.filter(
  ...
  private applyBrightness(brightness: number): void {
    ...
    applyColorAdjustmentToMatrix(brightnessFilter, this.sprite.colorAdjustment, brightness);
  }
