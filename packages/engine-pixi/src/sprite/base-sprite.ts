...
> extends EventEmitter<T> {
  ...
  private animatOptions: Required<IAnimationOptions> | null = null;

  private _animations: IAnimation[] = [];

  /**
   * List of active animations.
   * When setting this property, raw objects are automatically converted to IAnimation instances.
   */
  public get animations(): IAnimation[] {
  ...
  public set animations(v: IAnimation[]) {
    ...
    this._animations = v.map((anim: any) => {
      ...
      if (anim && anim.type && anim.options) {
        try {
   ...
   *
   */
  setAnimation(keyFrame: TKeyFrameOptions, options: IAnimationOptions): void {
    ...
    this.animatOptions = Object.assign({}, this.animatOptions, {
  ...
  animate(time: number, target?: any): void {
    ...
    if (
      ...
      this.animatOptions == null ||
      time < this.animatOptions.delay
    )
      return;
  ...
  copyStateTo<T extends BaseSprite>(target: T) {
    ...
    target.animatOptions = this.animatOptions;
...
export function linearTimeFn(
  ...
  const progress =
    offsetTime / options.duration >= options.iterCount || offsetTime === options.duration
      ? 1
      : t / options.duration;
